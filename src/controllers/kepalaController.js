...
exports.renderDashboardKepala = async (req, res) => {
    try {
        // 1. Statistik status laporan (hanya yang sudah dikirim ke kepala)
        ...
        // 2. Persentase pasien terkendali (executive summary)
        const terkendaliRes = await pool.query(`
            SELECT
                COUNT(DISTINCT s.id_pasien)::int AS total_pasien,
                COUNT(DISTINCT CASE WHEN s.sistole < 140 AND s.diastole < 90 THEN s.id_pasien END)::int AS terkendali
            FROM skrining s
        ...
        const persenTerkendali = totalPasien > 0
            ? ((jumlahTerkendali / totalPasien) * 100).toFixed(1)
            : '0.0';

        // 3. Laporan menunggu (status = 'dikirim')
        const menungguRes = await pool.query(`
            SELECT l.id_laporan, l.status, l.dikirim_pada, l.catatan_tolak,
        ...
        // 4. Riwayat laporan (disahkan / ditolak) — 10 terbaru
        ...
        // Tambahkan nama_bulan ke setiap baris
        const formatBaris = (row) => ({
            ...row,
        ...
        res.render('kepala/dashboardkepala', {
            ...
            laporanMenunggu: menungguRes.rows.map(formatBaris),
            riwayatLaporan:  riwayatRes.rows.map(formatBaris),
        });

    } catch (err) {
        console.error('ERROR renderDashboardKepala:', err);
        res.status(500).render('partials/404', { message: 'Gagal memuat dashboard. Periksa koneksi database.' });
    }
};

// ============================================================
// PERSETUJUAN LAPORAN (Activity 30–33 & 33)
// ============================================================
exports.renderPersetujuan = async (req, res) => {
    try {
        ...
        // Distribusi per nagari untuk laporan yang dipilih (jika ada ?id=)
        ...
        if (req.query.id) {
            ...
            if (detRes.rows.length > 0) {
                ...
                // Distribusi per nagari untuk periode laporan tsb
        ...
        res.status(500).render('partials/404', { message: 'Gagal memuat halaman persetujuan.' });
    }
};

// POST: Setujui laporan (Activity 31)
exports.handleSetujuiLaporan = async (req, res) => {
    const { id_laporan } = req.params;
    try {
        // Cek apakah kolom disahkan_oleh ada di tabel laporan
...
// POST: Tolak laporan (Activity 32)
...
// ============================================================
// GRAFIK KUNJUNGAN & CAPAIAN (Activity 34 & 29)
// ============================================================
exports.renderGrafikKunjungan = async (req, res) => {
    try {
        // Data 12 bulan terakhir: jumlah skrining per bulan
        const kunjunganRes = await pool.query(`
            SELECT
                per.periode_bulan   AS bulan,
                per.periode_tahun   AS tahun,
        ...
        // Rekap per periode untuk tabel executive level (Activity 29)
        ...
        const rekapPeriode = rekapPeriodeRes.rows.map(row => ({
            ...
            persen_capaian: row.total_pasien > 0
                ? ((row.terkendali / row.total_pasien) * 100).toFixed(1)
                : '0.0'
        }));

        // Capaian per nagari (semua waktu)
        const nagariRes = await pool.query(`
            SELECT
                n.nama_nagari,
                COUNT(DISTINCT s.id_pasien)::int AS total_pasien,
        ...
        res.status(500).render('partials/404', { message: 'Gagal memuat grafik kunjungan.' });
    }
};